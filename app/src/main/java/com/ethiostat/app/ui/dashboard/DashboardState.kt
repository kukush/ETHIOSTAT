...
data class DashboardState(
    ...
    val selectedPeriod: TimePeriod = TimePeriod.DAILY,
    val selectedSourceFilter: AccountSourceType? = null,
    val selectedAccountSource: AccountSource? = null,
    val showNetBalance: Boolean = false,
    val ussdResponseText: String? = null,
    ...
    val mainBalance: List<BalancePackage>
        get() = balances.filter { it.packageType == PackageType.MAIN_BALANCE }
    ...
    val totalMainBalanceBirr: Double
        get() = mainBalance.sumOf { it.remainingAmount }
}
